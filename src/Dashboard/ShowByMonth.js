...
const ShowByMonth = () => {
    ...
    const { data: categoriesInfo = [], isLoading: categoriesIsLoading, refetch: categoriesRefetch } = useQuery({
        queryKey: ["categoriesByMonth", year, month],
        queryFn: async () => {
            const res = await fetch(`https://printers-server.vercel.app/categoriesByMonth?month=${month}&year=${year}`, {
                headers: {
                        ...
                        <div>
                            ...
                            <div className='flex justify-end '>
                                ...
                                <div className='lg:mr-20 mr-5 font-bold mb-5 text-primary'>
                                    <span className='lg:mr-20 mr-5'>Total Cost Amount on {month},  {year} :</span> {sum.toFixed(2)} /-
                                </div>
                            </div>
                        </div> : <div>
